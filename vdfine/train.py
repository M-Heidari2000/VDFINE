...
def train_vdfine(
    ...
    for update in range(config.num_updates):
        ...
        y, u, c, _, _ = train_replay_buffer.sample(
        ...
        if update % config.test_interval == 0:
            ...
            with torch.no_grad():
                ...
                y, u, c, _, _ = test_replay_buffer.sample(
                    batch_size=config.batch_size,
                    chunk_length=config.chunk_length,
                )
...
def train_z_decoder(
    ...
    for update in range(config.num_updates):
        ...
        for t in range(1, config.chunk_length):
            ...
            z_recon = z_decoder(current_q_x_sample)
            loss += nn.MSELoss()(z_recon, z[t])

        loss /= (config.chunk_length - 1)
        ...
        writer.add_scalar("train/loss", loss.item(), update+1)
        print(f"update step: {update+1}, train_loss: {loss.item()}")
        ...
        if update % config.test_interval == 0:
            ...
            y, u, c, z, _ = test_replay_buffer.sample(
                batch_size=config.batch_size,
                chunk_length=config.chunk_length,
            )
            ...
            writer.add_scalar("test/loss", loss.item(), update+1)
            print(f"update step: {update+1}, test_loss: {loss.item()}")
